...
console.group('Užduotys');
{
    ...
    {
        const wordsReducer = (short, word) => short + word[0];
        const createAbbreviation = (words) => {
            const short = words.reduce(wordsReducer, '');
            return short;
        };
        const dataArr = [
        ...
        dataArr.forEach((words) => console.log(`[${words.join(', ')}] -> ${createAbbreviation(words)}`));
    }
    console.groupEnd();
    console.group('3. Sukurkite ir tipais aprašykite funkciją, kuri saudaugintų visus number masyvo skaičius');
    {
        ...
        const multiplyArray = (arr) => arr.reduce((previousValue, currentValue) => previousValue * currentValue);
        number.forEach((arr) => console.log(`[${arr.join(', ')}] -> ${multiplyArray(arr)}`));
    }
    console.groupEnd();
}
console.groupEnd();
