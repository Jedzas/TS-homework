...
{
  const isAdult = (obj: Person): string => (obj.age >= 18 ? 'žmogus yra pilnametis' : 'žmogus yra nepilnametis');
...
{
  const isFullyDescribedPerson = (obj: Person): string => (obj.height && obj.weight ? 'Objektas turi ūgį ir svorį' : 'Objektas neturi arba ugio arba svorio arba abiejų');
...
{
  const createInitials = (obj: Person): string => `${obj.name[0]}.${obj.surname[0]}.`;
